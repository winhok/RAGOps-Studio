...
import type { Session, DocumentRecord, ChatAnswer, Trace, Evaluation } from './types.js';
const root = document.getElementById('app')!;
type Tab = 'ask' | 'documents' | 'trace' | 'evaluation';
...
const icons: Record<string, string> = { ask: '◈', documents: '▤', trace: '⌁', evaluation: '◫' };
const labels: Record<Tab, string> = { ask: 'Ask workspace', documents: 'Knowledge library', trace: 'Execution traces', evaluation: 'Evaluations' };
const escape = (value: unknown) => String(value ?? '').replace(/[&<>"']/g, c => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[c]!));
const label = (value: string) => value.replaceAll('_', ' ');
const score = (value: number | null) => value === null ? '—' : value.toFixed(4);
const pill = (text: string, type = 'neutral') => `<span class="pill ${type}">${escape(text)}</span>`;
...
function render() {
  ...
  <div class="nav-label">WORKSPACE</div><nav>${(['ask', 'documents', 'trace', 'evaluation'] as Tab[]).map(t => `<button data-nav="${t}" class="nav-item ${tab === t ? 'active' : ''}"><span>${icons[t]}</span>${labels[t]}${t === 'documents' ? `<small>${documents.length}</small>` : ''}</button>`).join('')}</nav>
...
function renderLogin() {
 ...
 <section class="login-form"><div class="login-card"><span class="eyebrow">WORKSPACE ACCESS</span><h2>Connect to your workspace</h2><p>Enter a credential issued by your workspace administrator.</p>${message()}<form id="login-form"><label for="token">Access credential</label><input type="password" id="token" name="token" autocomplete="off" required placeholder="Paste your workspace credential"><button class="button primary wide" ${busy ? 'disabled' : ''}>${busy ? 'Connecting…' : 'Connect workspace →'}</button></form><div class="login-help"><strong>First time running this installation?</strong><p>Initialize workspace identities and the sample corpus:</p><code>python scripts/bootstrap.py --with-sample-data</code><p>Your generated credentials are stored locally in<br><code>.secrets/access-credentials.txt</code>.</p></div></div></section></main>`;
    document.getElementById('login-form')?.addEventListener('submit', async (e) => { e.preventDefault(); const value = (document.getElementById('token') as HTMLInputElement).value.trim(); setCredential(value); await connect(); });
}
function askView() {
...
function timeline(t: Trace) { return `<div class="panel timeline"><div class="panel-heading"><h3>Execution path</h3><code>${escape(t.trace_id.slice(0, 15))}…</code></div>${t.events.map((e, i) => `<div class="timeline-step"><span>${String(i + 1).padStart(2, '0')}</span><div><strong>${escape(label(e.node))}</strong><p>${escape(e.message)}</p></div><span class="step-dot"></span></div>`).join('')}</div>`; }
...
function modalView() {
    ...
    if (modal.kind === 'edit') {
        ...
        const opts = (values: string[], selected: string) => values.map(v => `<option value="${escape(v)}" ${selected === v ? 'selected' : ''}>${escape(label(v))}</option>`).join('');
...
function bind() {
    root.querySelectorAll<HTMLElement>('[data-nav]').forEach(el => el.addEventListener('click', () => { tab = el.dataset.nav as Tab; render(); }));
    root.querySelectorAll<HTMLElement>('[data-question]').forEach(el => el.addEventListener('click', () => { query = questions[Number(el.dataset.question)][1]; render(); }));
    root.querySelectorAll<HTMLElement>('[data-action]').forEach(el => el.addEventListener('click', () => {
        switch (el.dataset.action) {
            case 'signout':
                ...
                break;
            case 'refresh':
                void task(async () => { documents = await api<DocumentRecord[]>('/api/documents'); });
                break;
            case 'new-document':
            ...
            case 'close-modal':
                ...
                break;
            case 'evaluate':
                void task(async () => { evaluation = await api<Evaluation>('/api/evaluate', { method: 'POST', body: JSON.stringify({ k: 5 }) }); });
                break;
        }
    }));
    root.querySelectorAll<HTMLElement>('[data-source]').forEach(el => el.addEventListener('click', () => void task(async () => { const doc = await api<DocumentRecord>(`/api/documents/${encodeURIComponent(el.dataset.source!)}/versions/${el.dataset.version}/source`); modal = { kind: 'source', document: doc }; })));
    root.querySelectorAll<HTMLElement>('[data-history]').forEach(el => el.addEventListener('click', () => void task(async () => { modal = { kind: 'history', documents: await api<DocumentRecord[]>(`/api/documents/${encodeURIComponent(el.dataset.history!)}/versions`) }; })));
    root.querySelectorAll<HTMLElement>('[data-edit]').forEach(el => el.addEventListener('click', () => void task(async () => { const doc = documents.find(d => d.id === el.dataset.edit)!; modal = { kind: 'edit', document: await api<DocumentRecord>(`/api/documents/${encodeURIComponent(doc.id)}/versions/${doc.version}/source`) }; })));
    root.querySelectorAll<HTMLElement>('[data-delete]').forEach(el => el.addEventListener('click', () => { if (!confirm('Remove this document from active retrieval? Its revision history is retained.'))
        return; void task(async () => { await api(`/api/documents/${encodeURIComponent(el.dataset.delete!)}?expected_version=${el.dataset.version}`, { method: 'DELETE' }); documents = await api('/api/documents'); notice = 'Document removed from active retrieval.'; }); }));
    root.querySelectorAll<HTMLElement>('[data-trace]').forEach(el => el.addEventListener('click', () => void task(async () => { trace = await api<Trace>('/api/traces/' + encodeURIComponent(el.dataset.trace!)); tab = 'trace'; })));
    document.getElementById('query-form')?.addEventListener('submit', e => { e.preventDefault(); query = (document.getElementById('query') as HTMLTextAreaElement).value.trim(); if (!query)
        return; if (busy) return; answer = null; trace = null; void task(async () => { answer = await api<ChatAnswer>('/api/chat', { method: 'POST', body: JSON.stringify({ query, top_k: 4 }) }); trace = await api<Trace>('/api/traces/' + answer.trace_id); }); });
    document.getElementById('query')?.addEventListener('input', e => { query = (e.target as HTMLTextAreaElement).value; });
    document.getElementById('trace-form')?.addEventListener('submit', e => { e.preventDefault(); const id = String(new FormData(e.target as HTMLFormElement).get('trace_id')).trim(); void task(async () => { trace = await api<Trace>('/api/traces/' + encodeURIComponent(id)); }); });
    document.getElementById('document-form')?.addEventListener('submit', e => {
        e.preventDefault();
        const form = new FormData(e.target as HTMLFormElement);
        void task(async () => {
            const id = String(form.get('id') || ''), version = String(form.get('expected_version') || ''), file = form.get('file') as File;
...
async function task(work: () => Promise<void>) { if (busy)
    return; busy = true; error = ''; notice = ''; const current = epoch; render(); try {
    await work();
}
catch (e) {
    if (current !== epoch)
...
async function connect() { busy = true; error = ''; render(); try {
    session = await api<Session>('/api/session');
    documents = await api<DocumentRecord[]>('/api/documents');
}
catch (e) {
    setCredential('');
    session = null;
...
document.addEventListener('keydown', e => { if (e.key === 'Escape' && modal && !busy) {
    modal = null;
    render();
} });
if (hasCredential())

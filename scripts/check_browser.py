...
env = {
    **os.environ,
    'RAGOPS_STATE_DIR': str(work / 'state'),
    'RAGOPS_PRINCIPALS_FILE': str(work / 'identities/principals.json'),
    'RAGOPS_GRAPH_ENGINE': 'python',
    'RAGOPS_MODEL_PROVIDER': 'local',
    'RAGOPS_EMBEDDING_PROVIDER': 'hash',
    'RAGOPS_RERANK_PROVIDER': 'lexical',
    'RAGOPS_VECTOR_BACKEND': 'local',
    'EMBEDDING_DIMENSIONS': '512',
    'RAGOPS_MAX_SEARCHES': '3',
    'HOST': '127.0.0.1',
    'PORT': str(port),
}
...
with sync_playwright() as p:
    ...
    if args.transport_bridge:
        # Browser networking can be restricted in some CI/sandbox environments.
        # Load the real Vite production bundle, while forwarding fetch() calls
        # through the same local API process. No RAG response is mocked here.
        ...
        script = re.search(r'<script[^>]+src="([^"]+\.js)"[^>]*></script>', html)
        if not script:
            raise RuntimeError('Unable to locate the Vite JavaScript bundle in frontend/dist/index.html')
        styles = re.findall(r'<link[^>]+href="([^"]+\.css)"[^>]*>', html)
        shell = re.sub(r'<script[^>]+src="[^"]+\.js"[^>]*></script>', '', html)
        shell = re.sub(r'<link[^>]+href="[^"]+\.css"[^>]*>', '', shell)
        ...
        page.evaluate("""() => {
          ...
          window.fetch = async (url, options = {}) => {
            const result = await window.localRequest(url, {
        ...
        for stylesheet in styles:
            page.add_style_tag(content=(dist / stylesheet.lstrip('/')).read_text())
        page.add_script_tag(type='module', content=(dist / script.group(1).lstrip('/')).read_text())
        page.wait_for_selector('#login-form')
    else:
        page.goto(base_url, wait_until='networkidle')

    page.fill('#token', tokens['agentic-admin'])
    ...
    page.locator('[data-question="3"]').click()
    page.locator('#query-form button').click()
    ...
    page.fill('#token', tokens['kb-support'])
    page.locator('#login-form button').click()
    page.wait_for_selector('#query-form')
    page.fill('#query', '财务负责人需要复核哪些信息？')
    page.locator('#query-form button').click()
    page.wait_for_selector('.answer-panel .pill:text-is("refused")')
    ...
    page.locator('[data-action="signout"]').first.click()
    page.fill('#token', tokens['kb-admin'])
    page.locator('#login-form button').click()
    page.wait_for_selector('#query-form')
    page.locator('[data-nav="documents"]').first.click()
    ...
    page.locator('[data-nav="ask"]').first.click()
    page.fill('#query', '退款金额超过多少元需要人工审核？')
    page.locator('#query-form button').click()
    page.wait_for_selector('.answer-panel .pill:text-is("answered")')
